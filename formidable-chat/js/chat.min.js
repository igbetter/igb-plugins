...
if(deleting){--currentAnswer;--numberOfRepeatedAnswers}if(numberOfRepeatedAnswers<=1){toggleClass(progress,"frm_hidden",true);toggleClass(removeButton,"frm_hidden",true)}else{progress.textContent=currentAnswer+"/"+numberOfRepeatedAnswers;toggleClass(progress,"frm_hidden",false);toggleClass(removeButton,"frm_hidden",false)}}function syncRepeaterNavigationButtonsAndLabels(navigation){var sourceButtonWrapper,sourceButton,showIcons,addButton,removeButton;sourceButtonWrapper=state.activeForm.querySelector(".frm_active_chat_field .frm_remove_form_row").closest(".frm_repeat_buttons");
sourceButton=sourceButtonWrapper.querySelector(".frm_add_form_row");showIcons=null!==sourceButtonWrapper.querySelector("svg");addButton=navigation.querySelector(".frm_chat_repeater_add");removeButton=navigation.querySelector(".frm_chat_repeater_remove");addButton.innerHTML="";if(showIcons)maybeCopyButtonIcon(addButton,sourceButtonWrapper.querySelector(".frm_add_form_row"));addButton.appendChild(document.createTextNode(getButtonText(sourceButton)));removeButton.innerHTML="";if(showIcons)maybeCopyButtonIcon(removeButton,
sourceButtonWrapper.querySelector(".frm_remove_form_row"));sourceButton=sourceButtonWrapper.querySelector(".frm_remove_form_row");removeButton.appendChild(document.createTextNode(getButtonText(sourceButton)))}function maybeCopyButtonIcon(button,sourceButton){var icon=sourceButton.querySelector("svg");if(icon)button.appendChild(icon.cloneNode(true))}function getButtonText(button){var clone,icon;clone=button.cloneNode(true);icon=clone.querySelector("svg");if(icon)icon.remove();return clone.textContent}
function currentRepeaterAnswer(){var count,currentRepeatedSection;currentRepeatedSection=getClosestRepeaterSectionParent(getActiveField());count=1;while(currentRepeatedSection.previousElementSibling&&isRepeatedSection(currentRepeatedSection.previousElementSibling)){++count;currentRepeatedSection=currentRepeatedSection.previousElementSibling}return count}function getNumberOfRepeatersAnswers(){return getClosestParent(getActiveField(),".frm_chat_section").querySelectorAll(getRepeaterSectionSelectionString()).length}
function getRepeaterSectionSelectionString(){return".frm_repeat_sec, .frm_repeat_inline, .frm_repeat_grid"}function isOnStartPage(){var activeField=getActiveField();return activeField&&hasClass(activeField,"frm_chat_start_page")}function maybeHideKeyboardInstructions(){var keyboardInstructions=getActiveChatWrapper().querySelector(".frm-key-instructions");if(keyboardInstructions)toggleClass(keyboardInstructions,"frm_hidden",true)}function handleDocumentReady(){addDatepickerListeners();addAutocompleteListeners();
each(document.querySelectorAll(".frm_form_field > .frm_dropzone"),function(element){var dropzoneListener;if("object"!==typeof element.dropzone)return;if(hasClass(element,"frm_single_upload"))element.dropzone.on("success",handleDropzoneUploadSuccess);dropzoneListener=function(){state.activeForm=getClosestParent(element,".frm_chat_form");maybeSyncProgress()};element.dropzone.on("complete",dropzoneListener);element.dropzone.on("removedfile",dropzoneListener)});setTimeout(syncProgressAfterConditionalLogicRuns,
1);jQuery(document).on("frmPageChanged",handlePageChange);jQuery(document).on("frmFormComplete",handleFormComplete);if("undefined"!==typeof frmApiListener)frmApiListener.on("scriptsLoaded",handleApiFormLoad)}function handlePageChange(event,form){checkIfFormIsConversationalAndMaybeReinitialize(form)}function handleFormComplete(event,form){checkIfFormIsConversationalAndMaybeReinitialize(form)}function handleApiFormLoad(data){var form;if(!data.form||"function"!==typeof data.form.querySelector)return;
form=data.form.querySelector(".frm_chat_form");if(!form)return;initializeForm(form,1)}function checkIfFormIsConversationalAndMaybeReinitialize(form){if(form.querySelector(".frm_chat_form"))reinitializeFormAfterAjaxEvent(form.id)}function reinitializeFormAfterAjaxEvent(formId){state.ignoringClickEvents=false;state.animatingFields=false;setTimeout(function(){var form=document.getElementById(formId);if(form)initializeForm(form.querySelector(".frm_chat_form"))},0)}function addDatepickerListeners(){each(document.querySelectorAll("input.frm_date"),
addDatepickerListener);jQuery(document).on("frmdates_date_changed",handleDatepickerChange)}function handleDatepickerChange(event,args){var datepicker=document.querySelector(args.triggerID);setActiveFormFromFieldElement(datepicker);if(args.inline&&getLastActiveInput()===datepicker)maybeAutoAdvance(datepicker)}function maybeAutoAdvance(input){var field=getClosestParent(input,".frm_active_chat_field");if(!fieldIsInRepeater(field))continueChat()}function setActiveFormFromFieldElement(fieldElement){state.activeForm=
getClosestParent(fieldElement,".frm_chat_form")}function addAutocompleteListeners(){jQuery(document).on("change",".frm_chzn, .frm_slimselect",handleAutocompleteChange)}function handleAutocompleteChange(){setActiveFormFromFieldElement(this);if("multiple"!==this.getAttribute("multiple")&&getLastActiveInput()===this)maybeAutoAdvance(this)}function syncProgressAfterConditionalLogicRuns(){var activeForm=state.activeForm;each(getChatForms(),function(form){state.activeForm=form;maybeSyncProgress();toggleChatElements()});
state.activeForm=activeForm}function getClosestParent(element,selector){var activeParent=element.parentNode;while(activeParent&&"function"===typeof activeParent.matches){if(activeParent.matches(selector))return activeParent;activeParent=activeParent.parentNode}return false}function getClosestRepeaterSectionParent(field){return getClosestParent(field,getRepeaterSectionSelectionString())}function addDatepickerListener(datepicker){var $datepicker,addOnSelectToDatepicker;$datepicker=jQuery(datepicker);
addOnSelectToDatepicker=function(){var onSelect,handleDatepickerOnSelect;onSelect=$datepicker.datepicker("option","onSelect");handleDatepickerOnSelect=function(dateText,instance){if("function"===typeof onSelect)onSelect(dateText,instance);setActiveFormFromFieldElement(datepicker);if(getLastActiveInput()===datepicker)maybeAutoAdvance(datepicker)};$datepicker.datepicker("option","onSelect",handleDatepickerOnSelect);$datepicker.datepicker("show")};$datepicker.one("focusin",function(){setTimeout(addOnSelectToDatepicker,
...
activeForm,previousFields,activeRepeater,shouldUpdateCount;activeFields=getActiveFields();activeField=getActiveField(true);previousFields=getPreviousFields(activeField);if(fieldIsInRepeater(activeField))activeRepeater=getClosestParent(activeField,".frm_chat_section");if(previousFields.length){each(previousFields,function(previousField,index){toggleField(previousField,true,0===index,"right")});toggleField(activeFields,false,false,"right");maybeHideSection(activeField,"right");animateChatWrapper("right")}else{activeForm=
getClosestParent(activeField,".frm-show-form");if(thereIsAPreviousPageToGoBackTo(activeForm)){goToPreviousPage(activeForm);return}}tabState.currentIndex=0;toggleChatElements();hideElementsOnStartPage();shouldUpdateCount=true;if(hasClass(getActiveField(),"frm_chat_start_page"))shouldUpdateCount=false;else if(activeRepeater&&activeRepeater===getClosestParent(getActiveField(),".frm_chat_section"))shouldUpdateCount=false;if(shouldUpdateCount)updateQuestionNumber(-1);syncFinalQuestionClass();syncRepeaterNavigation();
maybeSyncProgress()}function thereIsAPreviousPageToGoBackTo(form){return"function"===typeof form.querySelector&&null!==form.querySelector("input.frm_next_page")}function goToPreviousPage(form){var nextPageInput;nextPageInput=form.querySelector("input.frm_next_page");nextPageInput.value=nextPageInput.id.replace("frm_next_p_","");submitForm(form)}function getPreviousFields(field){var fieldSection,previousField,previousFields,previousFieldSize,rowSize,currentField,currentSize;fieldSection=getFieldSection(field);
if(fieldSection&&fieldIsInRepeater(field)){if(fieldIsInFirstRepeatedSection(field))return getPreviousFields(fieldSection);return getClosestRepeaterSectionParent(field).previousElementSibling.querySelectorAll(".frm_inactive_chat_field")}previousField=field.previousElementSibling;while(shouldSkipElement(previousField))previousField=previousField.previousElementSibling;if(!previousField)if(hasClass(field,"frm_chat_section"))return getFieldsFromPreviousRepeatedSection();else if(fieldSection)return getPreviousFields(fieldSection);
else return[];if(!hasClass(previousField,"frm_inactive_chat_field")){if(!getFieldSection(previousField))return[];if(hasClass(previousField,"frm_embed_form_container"))return previousFields.concat(getPreviousFields({previousElementSibling:querySelectorLast(previousField,".frm_inactive_chat_field"),parentNode:previousField,className:"frm0"}));return getPreviousFields(getFieldSection(field))}previousFields=[previousField];if(hasClass(previousField,"frm_chat_section")){if(sectionIsRepeater(previousField))return previousFields.concat(querySelectorLast(previousField,
getRepeaterSectionSelectionString()).querySelectorAll(".frm_inactive_chat_field"));return previousFields}previousFieldSize=getFieldSize(previousField);if(12===previousFieldSize||hasClass(previousField,"frm_first"))return previousFields;rowSize=previousFieldSize;currentField=previousField;while(1){currentField=currentField.previousElementSibling;if(!currentField||"undefined"===typeof currentField.className||!hasClass(currentField,"frm_inactive_chat_field")||hasClass(currentField,"frm_chat_section"))break;
currentSize=getFieldSize(currentField);if(currentSize+rowSize>12)break;rowSize+=currentSize;previousFields.push(currentField);if(hasClass(currentField,"frm_first"))break}return previousFields}function querySelectorLast(element,selector){var elements=element.querySelectorAll(selector);return elements[elements.length-1]}function sectionIsRepeater(section){return null!==section.querySelector(".frm_add_form_row")}function fieldIsInFirstRepeatedSection(field){var repeatedSection=getClosestRepeaterSectionParent(field);
...
return turnstileResponse&&turnstileResponse.value!==""}function isInvisibleSection(element){return hasClass(element,"frm_invisible_section")}function isHiddenByClass(element){return hasClass(element,"frm_hidden")||hasClass(element,"frm_invisible")}function isHiddenSectionLabel(element){var hasHiddenSectionLabel;if(hasClass(element,"frm_hide_section"))return true;if(!hasClass(element,"frm_chat_section"))return false;hasHiddenSectionLabel=false;each(element.children,function(child){if("H3"===child.nodeName&&
hasClass(child,"frm_pos_hidden"))hasHiddenSectionLabel=true});return hasHiddenSectionLabel}function isLookupList(fieldContainer){var field,fieldID,lookupData;if("undefined"===typeof __FRMLOOKUP)return false;field=fieldContainer.querySelector("input");if(!field||"hidden"!==field.getAttribute("type"))return false;fieldID=getFieldIdFromNameAttribute(field);if(typeof __FRMLOOKUP[fieldID]==="undefined")return false;lookupData=__FRMLOOKUP[fieldID];return"data"===lookupData.inputType}function isSubmitButtonField(element){return null!==
element.querySelector(".frm_submit .frm_button_submit")}function getFieldIdFromNameAttribute(field){return field.getAttribute("name").replace("item_meta[","").replace("]","").replace("[]","")}function isDynamicListLoading(fieldContainer){var fieldId,rule;if("undefined"===typeof __frmDepDynamicFields||"undefined"===typeof __FRMRULES)return false;fieldId=fieldContainer.id.replace("frm_field_","").replace("_container","");if(-1===__frmDepDynamicFields.indexOf(fieldId)||!__FRMRULES.hasOwnProperty(fieldId))return false;
rule=__FRMRULES[fieldId];if("data"!==rule.fieldType)return false;return null!==fieldContainer.querySelector(".frm-loading-img")}function fieldIsInRepeater(field){if(!field||!field.parentNode)return false;return isRepeatedSection(field.parentNode)&&false===getClosestParent(field,".frm_embed_form_container")}function queryAndToggleChatElements(on){queryAndToggleChatWrapperChild(".frm_continue_chat_wrapper",on);queryAndToggleChatWrapperChild(".frm_chat_arrows",on);queryAndToggleChatWrapperChild(".frm-key-instructions",
on)}function queryAndToggleChatWrapperChild(selector,on){var target=getActiveChatWrapper().querySelector(selector);if(target)target.style.display=on?"":"none"}function toggleField(field,toggleOn,focus,animationDirection){var repeaterSection,handleFieldAnimationComplete,animationArgs,formWidth;if("undefined"===typeof field.className){each(field,function(currentField){toggleField(currentField,toggleOn,focus,animationDirection)});return}if(toggleOn){repeaterSection=getClosestRepeaterSectionParent(field);
if(repeaterSection)toggleClass(repeaterSection,"frm_hidden_repeater_section",false);else maybeAddSectionHeader(field)}if("undefined"===typeof animationDirection){if(toggleOn){field.style.opacity=1;field.style.left=0;toggleClass(field,"frm_active_chat_field",true);toggleClass(field,"frm_inactive_chat_field",false);if(focus)focusInputInField(field)}else{toggleClass(field,"frm_active_chat_field",false);toggleClass(field,"frm_inactive_chat_field",true)}return}state.animatingFields=true;state.ignoringClickEvents=
true;handleFieldAnimationComplete=function(){toggleClass(field,"frm_animating_chat_field",false);field.style.transform="";state.animatingFields=false;state.ignoringClickEvents=false;if(focus)focusInputInField(field)};animationArgs={};formWidth=jQuery(field).closest(".frm_chat_form").outerWidth();if("left"===animationDirection)if(toggleOn){field.style.left=formWidth+"px";animationArgs.left=0}else{field.style.left=0;animationArgs.left=-formWidth}else if("right"===animationDirection)if(toggleOn){field.style.left=
"-"+formWidth+"px";animationArgs.left=0}else{field.style.left=0;animationArgs.left=jQuery(field).outerWidth()}if(toggleOn){field.style.opacity=0;animationArgs.opacity=1}else{field.style.opacity=1;animationArgs.opacity=0}if(toggleOn){field.style.top="";field.style.width="";toggleClass(field,"frm_animating_chat_field",true);toggleClass(field,"frm_active_chat_field",true);toggleClass(field,"frm_inactive_chat_field",false);if(field.querySelector(".frm_likert"))window.dispatchEvent(new Event("resize"))}else{if(hasClass(field.parentNode,
"frm_chat_section"))field.style.top=getFieldInSectionTopValue(field);jQuery(field).width(jQuery(field).width());if(hasClass(field.previousElementSibling,"frm_animating_chat_field")&&hasClass(field.previousElementSibling,"frm_inactive_chat_field"))field.style.transform="translateX("+getTranslateXValue(field)+"px)";toggleClass(field,"frm_animating_chat_field",true);toggleClass(field,"frm_inactive_chat_field",true);toggleClass(field,"frm_active_chat_field",false)}jQuery(field).animate(animationArgs,
500,handleFieldAnimationComplete)}function maybeAddSectionHeader(field){var sectionIdElement,sectionId,section,sectionHeader,leftPadding,bottomPadding,clone;if(hasClass(field,"frm_section_heading")){maybeAddSectionFieldLabel(field);return}sectionIdElement=field.querySelector("[data-sectionid]");if(!sectionIdElement)return;sectionId=parseInt(sectionIdElement.getAttribute("data-sectionid"));if(!sectionId)return;section=document.getElementById("frm_field_"+sectionId+"_container");if(!section)return;
sectionHeader=section.querySelector("h3");if(!sectionHeader)return;leftPadding=window.getComputedStyle(sectionHeader).getPropertyValue("padding-left");bottomPadding=window.getComputedStyle(sectionHeader).getPropertyValue("padding-bottom");field.style.setProperty("--section-padding",leftPadding);clone=sectionHeader.cloneNode(true);clone.style.paddingBottom=0;field.style.paddingBottom=bottomPadding;toggleClass(field,"frm_section_heading",true);field.insertBefore(clone,field.firstChild)}function maybeAddSectionFieldLabel(field){var fieldLabel,
heading;if(!hasClass(field,"frm_chat_section"))return;fieldLabel=field.querySelector("label.frm_chat_section_label");if(fieldLabel)return;heading=field.querySelector("h3");if(!heading)return;fieldLabel=document.createElement("label");fieldLabel.className="frm_primary_label frm_chat_section_label";fieldLabel.textContent=heading.textContent;field.insertBefore(fieldLabel,field.firstChild)}function animateChatWrapper(direction){var wrapper,formWidth,animationArgs;wrapper=getActiveChatWrapper();formWidth=
jQuery(state.activeForm).outerWidth();wrapper.style.opacity=0;animationArgs={opacity:1};if("left"===direction){wrapper.style.left=formWidth+"px";animationArgs.left=0}else{wrapper.style.left=-formWidth+"px";animationArgs.left=0}jQuery(wrapper).animate(animationArgs,500)}function getFieldInSectionTopValue(field){var section,header,headerHeight;section=field.parentNode;headerHeight=0;header=section.querySelector("h3");if(header&&!hasClass(header,"frm_pos_none")){headerHeight=getStyleProperyInPixels(section,
"marginTop");headerHeight+=jQuery(header).outerHeight()+getStyleProperyInPixels(header,"marginBottom")}return headerHeight+"px"}function getStyleProperyInPixels(element,key){var style=element.currentStyle||window.getComputedStyle(element);return parseInt(style[key].replace("px",""))}function getTranslateXValue(field){var totalWidth,columnSize,gapSize,sizeOfPreviousFields,translateXValue;totalWidth=jQuery(getClosestParent(field,".frm_chat_form")).width();columnSize=totalWidth*.78/12;gapSize=(totalWidth-
columnSize*12)/11;sizeOfPreviousFields=0;jQuery(field).prevAll(".frm_animating_chat_field.frm_inactive_chat_field").each(function(){sizeOfPreviousFields+=getFieldSize(this)});translateXValue=(columnSize+gapSize)*sizeOfPreviousFields;return translateXValue}function handleClickEvents(event){var target,nodeType;if(state.ignoringClickEvents)return;target=event.target;if(hasClass(target,"frm_label_button_container")){if(radioButtonSiblingIsEnabled(target))handleRadioButtonClick(event)}else{nodeType=getNodeType(target);
if("input"===nodeType&&"radio"===target.getAttribute("type")){if(!clickWasTriggeredByArrowKeys(event))handleRadioButtonClick(event)}else if("option"===nodeType&&!hasClass(target,"frm_other_trigger"))handleDropdownClick(event);else if("input"===nodeType&&"checkbox"===target.getAttribute("type")){state.activeForm=getClosestParent(target,".frm_chat_form");maybeSyncProgress()}}}function radioButtonSiblingIsEnabled(labelContainer){var labelParent,radioButton;labelParent=labelContainer.closest("label");
if(!labelParent)return true;radioButton=labelParent.querySelector('input[type="radio"]');return!radioButton||!radioButton.disabled}function clickWasTriggeredByArrowKeys(event){return 0===event.clientX&&0===event.clientY}function getNodeType(element){return element.nodeName.toLowerCase()}function handleRadioButtonClick(event){setTimeout(getRadioButtonClickHandler(event),0)}function getRadioButtonClickHandler(event){return function(){var target,lastActiveInput,isOtherOption,radioInput,radio,otherOption;
target=event.target;setActiveFormFromFieldElement(target);lastActiveInput=getLastActiveInput();isOtherOption=lastActiveInput&&hasClass(lastActiveInput,"frm_other_input");if(!isOtherOption){if(!lastActiveInput||"input"!==getNodeType(lastActiveInput)||"radio"!==lastActiveInput.getAttribute("type"))return;radioInput=getClosestParent(target,".frm_form_field").querySelector('input[type="radio"]');if(!radioInput||lastActiveInput.getAttribute("name")!==radioInput.getAttribute("name"))return}radio=getClosestParent(target,
".frm_radio");if(radio){otherOption=radio.querySelector("input.frm_other_input");if(otherOption&&!hasClass(otherOption,"frm_pos_none")){state.activeForm=getClosestParent(radio,".frm_chat_form");toggleContinueButton(true);enableNextArrow();return}}maybeAutoAdvance(target)}}function enableNextArrow(){var nextArrow=getActiveChatWrapper().querySelector(".frm_chat_next_arrow");if(!nextArrow)return;toggleClass(nextArrow,"frm_disabled_arrow",false)}function handleDropdownClick(event){var select=event.target.parentNode;
setActiveFormFromFieldElement(select);if(getLastActiveInput()===select&&"multiple"!==select.getAttribute("multiple"))maybeAutoAdvance(select);else maybeSyncProgress()}function getLastActiveInput(){var visibleInputs=getVisibleInputs();return visibleInputs.length?visibleInputs[visibleInputs.length-1]:false}function handleChangeEvents(event){var target,nodeType,type,callback;target=event.target;setActiveFormFromFieldElement(target);if(false===state.activeForm)return;setTimeout(toggleChatElements,0);
nodeType=getNodeType(target);type=target.getAttribute("type");callback=function(){maybeAutoAdvance(target)};if("input"===nodeType&&"checkbox"===type&&target.checked&&hasClass(target.parentElement,"frm_switch_block"))setTimeout(callback,500);else if("input"===nodeType&&"file"===type&&getClosestParent(target,".frm_single_upload")&&target.files.length&&getLastActiveInput()===getClosestParent(target,".frm_active_chat_field"))callback()}function handleInputEvents(event){var target,nodeType,type;target=
event.target;setActiveFormFromFieldElement(target);if(false===state.activeForm)return;nodeType=getNodeType(target);type=target.getAttribute("type");if("textarea"===nodeType||"input"===nodeType&&-1===["radio","checkbox"].indexOf(type))maybeSyncProgress()}function hasClass(element,targetClass){var className=" "+element.className+" ";return-1!==className.indexOf(" "+targetClass+" ")}function each(items,callback){var index,length;length=items.length;for(index=0;index<length;index++)callback(items[index],
index)}function handleKeyEvent(event){var code;if(state.ignoringClickEvents)return;event=event||window.event;if("undefined"!==typeof event.key)code=event.key;else if("undefined"!==typeof event.keyCode)switch(event.keyCode){case 9:code="Tab";break;case 13:code="Enter";break}switch(code){case "Tab":handleTabEvent(event);break;case "Enter":handleEnterEvent(event);break}}function handleTabEvent(event){var movingForward,activeElement,visibleInputs,numberOfVisibleInputs,currentInputIndex,preventTabbingToNextField;
movingForward=!event.shiftKey;activeElement=event.target;if(hasClass(activeElement,"frm_verify")){setActiveFormFromFieldElement(activeElement);if(!fieldIsInRepeater(getActiveField()))continueChat();return}if(!isAFormidableInput(activeElement))return;setActiveFormFromFieldElement(document.activeElement);if(document.activeElement!==activeElement)return;if(fieldIsInRepeater(getClosestParent(document.activeElement,".frm_active_chat_field"))||getClosestParent(document.activeElement,".frm_likert"))return;
if("keydown"===event.type){visibleInputs=getVisibleInputs();tabState.visibleInputs=visibleInputs}else visibleInputs=tabState.visibleInputs;numberOfVisibleInputs=visibleInputs.length;currentInputIndex=-1;each(visibleInputs,function(input,index){if(input===activeElement)currentInputIndex=index});if(-1===currentInputIndex){if(!movingForward&&hasClass(activeElement,"frm_switch")&&!jQuery(getClosestParent(activeElement,".frm_active_chat_field")).prevAll(".frm_active_chat_field").length){event.preventDefault();
